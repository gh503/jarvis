...
Release qualification runs the same backup and restore entrypoints against
isolated state generated by the real pairing, session, and retained-event
stores:
...
The rehearsal rotates one device credential, revokes another device, backs up
Harness and Jarvis state, rejects an incompatible archive without replacing a
sentinel destination, and restores the valid archive into empty data roots. It
then checks every archived payload byte, private file permissions, active and
revoked credential behavior, access-session behavior, retained-event replay,
and absence of raw credentials and tokens from the archive.

This is an isolated automated rehearsal. It does not copy a real owner's
Keychain or model credentials, which are intentionally outside the archive and
must be reconfigured on a replacement Mac.
